...
type TransactionQuery struct {
	...
	withRecurringInstance *RecurringInstanceQuery
	withTransferPair      *TransactionQuery
	withTransferPairOf    *TransactionQuery
	withFKs               bool
...
func (_q *TransactionQuery) QueryAccount() *AccountQuery {
...
// QueryRecurringInstance chains the current query on the "recurring_instance" edge.
func (_q *TransactionQuery) QueryRecurringInstance() *RecurringInstanceQuery {
...
// QueryTransferPair chains the current query on the "transfer_pair" edge.
func (_q *TransactionQuery) QueryTransferPair() *TransactionQuery {
...
// QueryTransferPairOf chains the current query on the "transfer_pair_of" edge.
func (_q *TransactionQuery) QueryTransferPairOf() *TransactionQuery {
	query := (&TransactionClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withRecurringInstance: _q.withRecurringInstance.Clone(),
		withTransferPair:      _q.withTransferPair.Clone(),
		withTransferPairOf:    _q.withTransferPairOf.Clone(),
...
func (_q *TransactionQuery) WithAccount(opts ...func(*AccountQuery)) *TransactionQuery {
	query := (&AccountClient{config: _q.config}).Query()
...
// WithRecurringInstance tells the query-builder to eager-load the nodes that are connected to
// the "recurring_instance" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TransactionQuery) WithRecurringInstance(opts ...func(*RecurringInstanceQuery)) *TransactionQuery {
	query := (&RecurringInstanceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRecurringInstance = query
	return _q
}

// WithTransferPair tells the query-builder to eager-load the nodes that are connected to
// the "transfer_pair" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TransactionQuery) WithTransferPair(opts ...func(*TransactionQuery)) *TransactionQuery {
	query := (&TransactionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTransferPair = query
	return _q
}

// WithTransferPairOf tells the query-builder to eager-load the nodes that are connected to
// the "transfer_pair_of" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TransactionQuery) WithTransferPairOf(opts ...func(*TransactionQuery)) *TransactionQuery {
	query := (&TransactionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTransferPairOf = query
	return _q
}
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	var (
		...
		loadedTypes = [5]bool{
			_q.withAccount != nil,
			_q.withCategory != nil,
			_q.withRecurringInstance != nil,
			_q.withTransferPair != nil,
			_q.withTransferPairOf != nil,
		}
	)
	if _q.withAccount != nil || _q.withCategory != nil || _q.withRecurringInstance != nil || _q.withTransferPair != nil || _q.withTransferPairOf != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withRecurringInstance; query != nil {
		if err := _q.loadRecurringInstance(ctx, query, nodes, nil,
			func(n *Transaction, e *RecurringInstance) { n.Edges.RecurringInstance = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTransferPair; query != nil {
		if err := _q.loadTransferPair(ctx, query, nodes, nil,
			func(n *Transaction, e *Transaction) { n.Edges.TransferPair = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTransferPairOf; query != nil {
		if err := _q.loadTransferPairOf(ctx, query, nodes, nil,
			func(n *Transaction, e *Transaction) { n.Edges.TransferPairOf = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TransactionQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Account)) error {
	...
	for i := range nodes {
		if nodes[i].account_transactions == nil {
			continue
		}
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionQuery) loadRecurringInstance(ctx context.Context, query *RecurringInstanceQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *RecurringInstance)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].recurring_instance_transaction == nil {
			continue
		}
		fk := *nodes[i].recurring_instance_transaction
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(recurringinstance.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "recurring_instance_transaction" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionQuery) loadTransferPair(ctx context.Context, query *TransactionQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Transaction)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].transaction_transfer_pair == nil {
			continue
		}
		fk := *nodes[i].transaction_transfer_pair
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "transaction_transfer_pair" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionQuery) loadTransferPairOf(ctx context.Context, query *TransactionQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Transaction)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].transaction_transfer_pair == nil {
