...
type AccountQuery struct {
	...
	withRecurringItems *RecurringItemQuery
...
// QueryRecurringItems chains the current query on the "recurring_items" edge.
func (_q *AccountQuery) QueryRecurringItems() *RecurringItemQuery {
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withRecurringItems: _q.withRecurringItems.Clone(),
...
// WithRecurringItems tells the query-builder to eager-load the nodes that are connected to
// the "recurring_items" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithRecurringItems(opts ...func(*RecurringItemQuery)) *AccountQuery {
	query := (&RecurringItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRecurringItems = query
	return _q
}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withRecurringItems; query != nil {
		if err := _q.loadRecurringItems(ctx, query, nodes,
			func(n *Account) { n.Edges.RecurringItems = []*RecurringItem{} },
			func(n *Account, e *RecurringItem) { n.Edges.RecurringItems = append(n.Edges.RecurringItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AccountQuery) loadSnapshots(ctx context.Context, query *AccountSnapshotQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccountSnapshot)) error {
	...
	for _, n := range neighbors {
		fk := n.account_snapshots
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_snapshots" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_snapshots" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *AccountQuery) loadRecurringItems(ctx context.Context, query *RecurringItemQuery, nodes []*Account, init func(*Account), assign func(*Account, *RecurringItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.RecurringItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.RecurringItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_recurring_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_recurring_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_recurring_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
