...
func (_q *CategoryQuery) QueryRecurringItems() *RecurringItemQuery {
...
func (_q *CategoryQuery) WithRecurringItems(opts ...func(*RecurringItemQuery)) *CategoryQuery {
	query := (&RecurringItemClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withRecurringItems; query != nil {
		if err := _q.loadRecurringItems(ctx, query, nodes,
			func(n *Category) { n.Edges.RecurringItems = []*RecurringItem{} },
			func(n *Category, e *RecurringItem) { n.Edges.RecurringItems = append(n.Edges.RecurringItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CategoryQuery) loadRecurringItems(ctx context.Context, query *RecurringItemQuery, nodes []*Category, init func(*Category), assign func(*Category, *RecurringItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.RecurringItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.RecurringItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.category_recurring_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_recurring_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_recurring_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
